...
This project contains work done for UMass lowell capstone industry program for industry sponsors at General Dynamics - Bath Iron Works.
It utilizes Unit's ARkit plugin, and in specific ARKit 2/3 features.
...
Speed Warning (`Assets/Scripts/SpeedTest`): Contains Speed Warning Application which tracks IMU Acceleration and gives a warning based on the User's Speed

Desktop InfoTagger (`Assets/Scripts/DesktopInfoTagger`): Contains python code for an application to transform coordinates generated by InfoTagger Mobile to be analyzed on a 2D planview WRT Ship Coordinates. This was called Task 7 in the SPRINT documents.
